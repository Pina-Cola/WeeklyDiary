...
\begin{itemize}
	...
	\item \fbox{%
		\parbox{\linewidth}{%
			Changing the red value with the slider works while the video is playing! YAY! \includegraphics[height=0.32cm]{success.png}
		}%
	}
...
\begin{itemize}
	...
	\item Writing on thesis:
	\begin{itemize}
		\item[$\square$] Comparing different MLT filters 
		\item[$\square$] Introduce RGB
		\item[$\square$] Add pre selection of suitable filters?
	\end{itemize}
	...
	\item[$\square$] List of questions for meeting with supervisor
		\begin{itemize}
			\item Footcites when reference used more often?
			\item What to do with it for thesis? Comparing with offline version still suitable?
			\item Preselection of filters?
		\end{itemize}
	
	\item[$\square$] Meeting supervisor at university
	
	
	% different video file sizes: comparison
	% memory usage comparison
	% comparing results with local or offline version of melt -> accuary
	% are the same frames produced?
	% ffmpeg
