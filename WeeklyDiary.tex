...
\begin{itemize}
	...
	\item Backend: Adding red value with \texttt{"av.rs=$\%$.1f" $\%$ RED\_VALUE} to have a variable that can (hopefully) be changed from the frontend and then the red value will maybe update when it is changes (in \texttt{jit-webrtc/jit/local\_melt.py})
\end{itemize}
