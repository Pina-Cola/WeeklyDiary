...
\begin{itemize}
	...
	\item On the trail of print statements - disappeared or user error? (Aka figuring out where the logging info and print statements are printed to get more insight of the code)
		\item Fun fact (for later use): Find the name of your docker container with \texttt{docker ps} and get info with \texttt{docker logs --follow <container-name>} (but the missing info cannot be found there either so far)
\end{itemize}
